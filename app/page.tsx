...
const CATCHES = [
  { icon: "✈️", title: "Flight-arrival requirement", desc: "Airport rates requiring proof of inbound flight — the #1 cause of desk denials.", color: "#c0303c" },
  { icon: "💳", title: "Credit card only", desc: "Debit cards refused at counter. Discovered after you've already paid online.", color: "#f59e0b" },
  { icon: "💰", title: "Deposit pre-auth holds", desc: "£500–£2,000 holds that freeze your account for days, undisclosed upfront.", color: "#c0303c" },
  ...
  { icon: "❌", title: "Non-refundable terms", desc: "Zero recourse if denied at desk or unable to collect.", color: "#c0303c" },
];
...
export default function Home() {
  return (
    <div style={{ fontFamily: "var(--font-inter), system-ui, sans-serif", minHeight: "100vh", background: "#f8f7f4" }}>
      ...
      {/* ── Nav ── */}
      <nav style={{ background: "rgba(13,13,30,0.96)", backdropFilter: "blur(12px)", padding: "0 24px", borderBottom: "2px solid #c0303c", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", height: 62 }}>
          <a href="/" style={{ display: "flex", alignItems: "center", gap: 8, textDecoration: "none" }}>
            ...
            <span style={{ color: "#fff", fontSize: 22, fontWeight: 900, letterSpacing: "-0.5px" }}>
              Rental<span style={{ color: "#c0303c" }}>Truth</span>
            </span>
            <span style={{ background: "#c0303c", color: "#fff", fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 4, letterSpacing: "0.05em" }}>BETA</span>
          </a>
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", background: "linear-gradient(160deg, #0d0d1e 0%, #1a1a2e 50%, #0d0d1e 100%)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 24px", textAlign: "center", position: "relative", zIndex: 1, width: "100%" }}>
          <div style={{ display: "inline-block", background: "rgba(230,57,70,0.12)", border: "1px solid rgba(230,57,70,0.3)", borderRadius: 20, padding: "5px 16px", fontSize: 11, fontWeight: 700, color: "#c0303c", marginBottom: 28, letterSpacing: "0.1em", textTransform: "uppercase" as const }}>
          ...
          <h1 style={{ color: "#fff", fontSize: "clamp(36px, 6vw, 72px)", fontWeight: 900, lineHeight: 1.06, margin: "0 0 24px", letterSpacing: "-2px", maxWidth: 900, marginLeft: "auto", marginRight: "auto" }}>
            ...
            <span style={{ color: "#c0303c" }}>before they cost you money.</span>
          </h1>
          ...
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(230,57,70,0.08)", border: "1px solid rgba(230,57,70,0.22)", borderRadius: 24, padding: "8px 18px", marginBottom: 40 }}>
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#c0303c", display: "inline-block", boxShadow: "0 0 8px #c0303c", flexShrink: 0 }} />
            <span style={{ fontSize: 13, fontWeight: 600, color: "#f1f5f9" }}>Join <strong style={{ color: "#c0303c" }}>0+</strong> travellers protected</span>
          </div>

          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" as const }}>
            <a href="#tool" style={{ background: "#c0303c", color: "#fff", fontWeight: 700, fontSize: 16, padding: "16px 38px", borderRadius: 10, textDecoration: "none", boxShadow: "0 4px 22px rgba(230,57,70,0.45)" }}>
              Analyse My Booking →
            </a>
        ...
        <div style={{ position: "absolute", bottom: 32, left: "50%", transform: "translateX(-50%)", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, opacity: 0.35, pointerEvents: "none" }}>
          ...
          <span style={{ color: "#c0303c", fontSize: 16 }}>↓</span>
        </div>
      </div>
        ...
        <div className="stats-bar-inner" style={{ maxWidth: 1100, margin: "0 auto", display: "flex", justifyContent: "center", flexWrap: "wrap" }}>
          {STATS.map(({ stat, label }, i) => (
            <div key={stat} style={{ textAlign: "center", padding: "0 44px", borderRight: i < STATS.length - 1 ? "1px solid rgba(230,57,70,0.35)" : "none" }}>
              <div style={{ fontSize: 32, fontWeight: 900, color: "#c0303c", letterSpacing: "-1px" }}>{stat}</div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px" }}>
        ...
        <section id="problem" style={{ padding: "80px 0 0" }}>
          <div className="hover-card" style={{ background: "#fff", border: "1.5px solid #e2e8f0", borderRadius: 16, padding: "44px 48px", position: "relative", overflow: "hidden" }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.1em", color: "#c0303c", marginBottom: 16 }}>Why We Built This</div>
            {/* Large decorative quote mark */}
            <div aria-hidden="true" style={{ position: "absolute", top: 12, left: 24, fontSize: 180, lineHeight: 1, color: "#c0303c", opacity: 0.07, fontFamily: "Georgia, serif", fontWeight: 900, userSelect: "none", pointerEvents: "none" }}>&ldquo;</div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, paddingTop: 24, marginTop: 24, borderTop: "1px solid #f1f5f9" }}>
              <div style={{ width: 44, height: 44, borderRadius: "50%", background: "#c0303c", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 900, fontSize: 17, flexShrink: 0 }}>R</div>
              <div>
          ...
          <div className="timeline-steps" style={{ display: "flex", alignItems: "flex-start" }}>
            {STEPS.map(({ step, icon, title, desc }, i) => (
              <div key={step} style={{ flex: 1, position: "relative", textAlign: "center", padding: "0 16px" }}>
                {i > 0 && (
                  <div className="timeline-arrow" style={{ position: "absolute", left: -4, top: 17, fontSize: 22, color: "#c0303c", opacity: 0.65, fontWeight: 700 }}>→</div>
                )}
                <div style={{ width: 48, height: 48, borderRadius: "50%", background: "#c0303c", color: "#fff", fontWeight: 900, fontSize: 19, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", boxShadow: "0 4px 16px rgba(230,57,70,0.38)" }}>{step}</div>
        ...
        <section style={{ padding: "80px 0 60px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" as const, gap: 10 }}>
            {FAQS.map(({ question, answer }) => (
              <div key={question} className="hover-card" style={{ background: "#fff", border: "1.5px solid #e2e8f0", borderLeft: "4px solid #c0303c", borderRadius: 12, padding: "22px 24px" }}>
      ...
      {/* ── Mobile floating CTA ── */}
      <a href="#tool" className="mobile-cta-btn" style={{ position: "fixed", bottom: 24, right: 20, zIndex: 100, background: "#c0303c", color: "#fff", fontWeight: 700, fontSize: 14, padding: "14px 22px", borderRadius: 30, textDecoration: "none", boxShadow: "0 4px 22px rgba(230,57,70,0.55)", alignItems: "center", gap: 8 }}>
        Try it free →
      </a>
