...
export default function AnalyserTool() {
  ...
  const tabStyle = (tab: string): React.CSSProperties => ({
    ...
    fontWeight: 600,
    border: "none",
    borderBottom: activeTab === tab ? "3px solid #e8501a" : "3px solid transparent",
    background: "none",
    cursor: "pointer",
  ...
  return (
    <div>
      ...
      <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #e2e8f0", overflow: "hidden", marginBottom: 28 }}>
        ...
        <div style={{ padding: "18px 24px", background: "#f8f9fb", borderTop: "1px solid #f1f5f9" }}>
          ...
          <button
            ...
            style={{
              width: "100%",
              background: isDisabled ? "#d1d5db" : "#e8501a",
              color: "#fff",
              border: "none",
              borderRadius: 10,
              padding: "16px 32px",
      ...
      {error && (
        <div style={{ background: "#fff1f2", border: "1.5px solid #fecdd3", borderLeft: "4px solid #e8501a", borderRadius: 10, padding: "14px 18px", color: "#881337", fontSize: 13, marginBottom: 16 }}>
          {error}
        </div>
      )}
